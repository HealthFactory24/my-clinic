...
export const pageLayerToCode = (
	...
	if (Array.isArray(layers)) {
		layers.forEach(layer => {
			...
			layerFuncIds.forEach(id => {
				functionMetadataIds.add(id);
			});
		});
	}
...
export const generateLayerCode = (
	...
	const resolvedIdentifiers =
		variableIdentifiers ??
		(variables.length > 0 ? generateVariableIdentifiers(variables) : undefined);
	...
	return `${indentation}<${layer.type}${generatePropsString(layer.props, variables, resolvedIdentifiers, functionIdentifiers)} />`;
};
	...
	const propsArray = Object.entries(props)
		...
		.map(([key, value]) => {
			...
			if (isVariableReference(value)) {
				...
				if (variable && resolvedIdentifiers) {
					// Function-type variables go in `functions`, other types in `variables`
					if (variable.type === "function") {
						propValue = `{functions.${resolvedIdentifiers.get(variable.id)}}`;
					} else {
						propValue = `{variables.${resolvedIdentifiers.get(variable.id)}}`;
					}
				} else {
...
const generateVariablePropsInterface = (
	...
	const variableTypes = regularVariables
		.map(variable => {
			...
			return `    ${resolvedIdentifiers?.get(variable.id)}: ${tsType};`;
		})
		.join("\n");
	...
	const functionVariableTypes = functionVariables.map(variable => {
		...
		if (funcDef?.typeSignature) {
			return `    ${resolvedIdentifiers?.get(variable.id)}: ${funcDef.typeSignature};`;
		}

		if (funcDef?.schema) {
			const typeString = zodSchemaToTypeString(funcDef.schema);
			return `    ${resolvedIdentifiers?.get(variable.id)}: ${typeString};`;
		}

		// Fallback to generic function type
		return `    ${resolvedIdentifiers?.get(variable.id)}: (...args: unknown[]) => unknown;`;
	});
	...
	for (const variable of functionVariables) {
		const identifier = resolvedIdentifiers?.get(variable.id);
		if (identifier) {
