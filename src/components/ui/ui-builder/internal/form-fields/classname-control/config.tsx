...
export function resolveClassNameControlConfig(
	profile?: ClassNameControlProfile
): {
	config: ConfigType;
	...
	const allowedClassSet = allowedClassNames ? new Set(allowedClassNames) : null;

	const filteredConfigEntries = Object.entries(CONFIG).flatMap(
		([key, item]) => {
			...
			const possibleTypes = item.possibleTypes.filter(possibleType =>
				possibleType === null
					? true
					: isTokenAllowed(possibleType, allowedClassSet)
			);
