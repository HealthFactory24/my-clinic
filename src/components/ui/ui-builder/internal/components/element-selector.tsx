...
export const ElementSelector: React.FC<ElementSelectorProps> = ({
	...
	return (
		<>
			<MeasureRange
				debug={false}
				onChange={handleBoundingRectChange}
			>
				{children}
			</MeasureRange>
			...
			{boundingRect && (
				...
				>
					<div
						className={cn(
							"fixed box-border cursor-default hover:border-2 hover:border-blue-300 hover:bg-blue-300/20 hover:shadow-blue-500/20 hover:shadow-md",
							isBeingDragged
								? "border-2 border-green-500 border-dashed bg-orange-50/20 opacity-70 shadow-lg"
								: isSelected
			...
			{isSelected && (
				...
				>
					<div className='flex items-center bg-blue-500'>
						...
						{!isBeingDragged && (
							<>
								<span className='cursor-default whitespace-nowrap px-[1px] text-white text-xs'>
									{layer.name
										?.toLowerCase()
								...
								<div className='ms-1 h-4 w-px bg-white/20' />
...
export const MeasureRange: React.FC<MeasureRangeProps> = ({
	...
	useLayoutEffect(() => {
		...
		return () => {
			...
			for (const cleanup of scrollListeners) {
				cleanup();
			}
